...
<x-layout>
    ...
    <style>
        ...
        .order-card {
            ...
            margin-bottom: 1rem;
            cursor: pointer;
            transition: box-shadow 0.2s ease-in-out, border-color 0.2s ease-in-out; /* Added border-color transition */
        }
        .order-card:hover {
            box-shadow: 0 4px 8px rgba(0,0,0,0.05);
        }
        .order-card.active { /* Style for when details are open */
             border-color: #a07d6a; /* Highlight border */
                    ...
                    @foreach($placedOrders as $index => $order)
                        ...
                        <div class="order-card"
                            ...
                            <div class="flex justify-between items-center">
                                <span class="font-semibold text-lg" style="color: #6b4f4f;">{{ $order['order_id'] ?? 'Unknown Order' }}</span>
                                <span class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($order['timestamp'])->format('M d, Y H:i') }}</span>
                            </div>
                            {{-- Order Footer --}}
                            <div class="flex justify-between items-center mt-2">
                                <span class="text-gray-700">Total: <span class="font-bold">IDR {{ number_format($order['total_amount'] ?? 0, 0, ',', '.') }}</span></span>
                                <span class="text-xs font-medium px-2 py-0.5 rounded {{ $order['status'] === 'Pending Payment' ? 'bg-yellow-200 text-yellow-800' : 'bg-gray-200 text-gray-800' }}">
                ...
                orderList.addEventListener('click', function(event) {
                    ...
                    if (!isCurrentlyActive) {
                        try {
                            ...
                            let detailsHtml = '<h4 class="text-md font-semibold mb-3 border-b pb-1" style="color: #8a6c5a;">Order Items:</h4>';
